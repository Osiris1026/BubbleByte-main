...
public class RobotContainer {
    ...
    public Command CoralOuttake(){
        return new SequentialCommandGroup(new InstantCommand(()-> c_CoralIntakeSubsystem.setSpeed(Constants.CoralIntakeConstants.OuttakeSpeed)), 
        new WaitCommand(Constants.CoralIntakeConstants.outtakeTime),
        new InstantCommand(()-> c_CoralIntakeSubsystem.setSpeed(0)));
    }
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
    ...
    public Command getAutonomousCommand() {
        return new SequentialCommandGroup(new InstantCommand(()-> s_Swerve.gyro.reset()), Nest(),  autoChooser.getSelected());
